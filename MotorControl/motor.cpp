...
// Motor::Motor(float shunt_conductance,
...
bool Motor::setup()
{
    ...
    float requested_gain = max_unity_gain_current / config_.requested_current_range; // [A*gain/A]
...
bool Motor::disarm()
{
    ...
    // this version doesn't have brake resister so no use
...
void Motor::current_meas_cb(uint32_t timestamp, std::optional<Iph_ABC_t> current)
{
    ...
    bool dc_calib_valid = (dc_calib_running_since_ >= config_.dc_calib_tau * 7.5f) && (abs(DC_calib_.phA) < max_dc_calib_) && (abs(DC_calib_.phB) < max_dc_calib_) && (abs(DC_calib_.phC) < max_dc_calib_);
...
//     if (config_.motor_type == MOTOR_TYPE_HIGH_CURRENT || config_.motor_type == MOTOR_TYPE_ACIM)
//     {
